    ...
    <AppLayout title="Профиль бизнеса">
        ...
        <form class="ui-scope max-w-2xl space-y-5 rounded-xl border border-slate-200 bg-white p-6 dark:border-white/10" @submit.prevent="submit">
            ...
            <div>
                ...
                <input v-model="form.name" type="text" placeholder="Барбершоп «Бруно»" class="w-full rounded-lg border border-slate-300 px-3 py-2 dark:border-white/15 dark:bg-white/5" />
                <p v-if="form.errors.name" class="mt-1 text-sm text-red-600">{{ form.errors.name }}</p>
            </div>

            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-200">Описание</label>
                <textarea v-model="form.description" rows="3" placeholder="Чем занимается бизнес, ключевые услуги и преимущества. Пара предложений." class="w-full rounded-lg border border-slate-300 px-3 py-2 dark:border-white/15 dark:bg-white/5" />
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input v-model="form.working_hours" type="text" placeholder="Пн–Пт 9:00–20:00, Сб 10:00–18:00" class="w-full rounded-lg border border-slate-300 px-3 py-2 dark:border-white/15 dark:bg-white/5" />
                </div>
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input v-model="form.address" type="text" placeholder="Москва, ул. Ленина 1, 2 этаж" class="w-full rounded-lg border border-slate-300 px-3 py-2 dark:border-white/15 dark:bg-white/5" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-200">Сайт</label>
                    <input v-model="form.website" type="text" placeholder="example.ru" class="w-full rounded-lg border border-slate-300 px-3 py-2 dark:border-white/15 dark:bg-white/5" />
            ...
            <div>
                ...
                <textarea v-model="form.escalation_note" rows="3" placeholder="Когда передавать диалог человеку. Например: клиент просит позвать администратора, жалоба, вопрос про возврат денег, нестандартная просьба." class="w-full rounded-lg border border-slate-300 px-3 py-2 dark:border-white/15 dark:bg-white/5" />
            ...
            <div class="flex items-center gap-3">
                <button
                    ...
                    class="rounded-lg bg-[#2E74B5] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#255f96] disabled:opacity-50"
                >
                    Сохранить
                </button>
