...
| GET/POST/DELETE | `/cabinet/broadcasts` (+ `…/{id}/run`, `…/{id}/cancel`) | `Cabinet\BroadcastController` | Рассылки по базе клиентов (auth + `tenant` + `plan:broadcasts` + раздел `broadcasts`). Создание (сейчас/по расписанию), запуск, снятие с расписания, удаление. |
