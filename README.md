...
| `App\Services\KnowledgeBaseService` | CRUD базы знаний (эмбеддинги/RAG — Фаза 3). **Готовые элементы БЗ по типам бизнеса** (`App\Support\KnowledgeTemplates`, тот же реестр `App\Enums\BusinessType` + «Общие»): `KnowledgeEntryController::index` отдаёт `templates`+`businessTypes`, фронт (`Cabinet/KnowledgeBase/Index.vue`) группирует карточки; клик предзаполняет форму создания (заголовок+текст с плейсхолдерами «…», бизнес дозаполняет и сохраняет — бот отвечает строго по заполненному). |
