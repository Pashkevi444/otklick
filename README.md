...
| Анонсы площадки (`App\Models\Announcement` + `App\Services\AnnouncementService` + `App\Enums\AnnouncementType`) | **Новости** платформы (отдельная лента «Обновлений» убрана): публикует супер-админ (`Admin\AnnouncementController`, `/admin/news`) — **форматированный текст с картинками** (rich-text редактор на TipTap, `resources/js/Components/RichTextEditor.vue`; загрузка картинок `POST /admin/announcements/images` → `App\Support\AnnouncementImageStorage`, public-диск; лёгкая санитизация HTML в сервисе, автор — доверенный СУ). В админке: пагинация, даты создания/публикации, предпросмотр «как увидит бизнес». Бизнес видит опубликованное в кабинете (`Cabinet\AnnouncementController`, `/cabinet/news`) — пагинированный список + **детальные страницы** (`…/news/{id}`), новые сверху. Анонсы глобальные (без tenant_id). **Прочтение — пер-тенант** (`announcement_reads`, RLS): открытие раздела/детали помечает прочитанным; непрочитанное подсвечивает пункт меню (shared-prop `announcementsUnread`). |
...
| `App\Services\KnowledgeBaseService` | CRUD базы знаний (эмбеддинги/RAG — Фаза 3). **Готовые элементы БЗ по типам бизнеса** (`App\Models\KnowledgeTemplate`, таблица `knowledge_templates`, глобальная; тот же реестр `App\Enums\BusinessType` + «Общие»; ~80 шт: 20 общих + по 10 на нишу): `KnowledgeEntryController::index` отдаёт `templates`+`businessTypes`, фронт (`Cabinet/KnowledgeBase/Index.vue`) группирует карточки за кнопкой; клик предзаполняет форму создания (заголовок+текст с плейсхолдерами «…», бизнес дозаполняет и сохраняет — бот отвечает строго по заполненному). Начальный набор засеян миграцией `create_template_tables`; правит супер-админ (`Admin\KnowledgeTemplateController`, `/admin/knowledge-templates`). Список записей БЗ пагинируется. |
