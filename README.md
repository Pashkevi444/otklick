...
| Анонсы площадки (`App\Models\Announcement` + `App\Services\AnnouncementService` + `App\Enums\AnnouncementType`) | **Новости и обновления** (патчи) платформы: публикует супер-админ (`Admin\AnnouncementController`, `/admin/news`, `/admin/updates`), видны всем бизнесам в кабинете (`Cabinet\AnnouncementController`, `/cabinet/news`, `/cabinet/updates`). Анонсы глобальные (без tenant_id). **Прочтение — пер-тенант** (`announcement_reads`, RLS): открытие раздела помечает показанные анонсы прочитанными; непрочитанное подсвечивает пункт меню (shared-prop `announcementsUnread`). Единая модель, тип (`news`/`update`) разводит две ленты. |
