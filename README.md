...
| GET/POST/PUT/DELETE | `/cabinet/scenarios` (+ `…/{flow}`, `…/{flow}/toggle`) | `Cabinet\FlowController` | Конструктор сценариев / no-code воронки (auth + `tenant` + `plan:flows` + раздел `scenarios`). Список/создание/правка/вкл-выкл/удаление сценариев бота. |
