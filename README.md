...
| Анонсы площадки (`App\Models\Announcement` + `App\Services\AnnouncementService` + `App\Enums\AnnouncementType`) | **Новости и обновления** (патчи) платформы: публикует супер-админ (`Admin\AnnouncementController`, `/admin/news`, `/admin/updates`) — **форматированный текст с картинками** (rich-text редактор на TipTap, `resources/js/Components/RichTextEditor.vue`; загрузка картинок `POST /admin/announcements/images` → `App\Support\AnnouncementImageStorage`, public-диск; лёгкая санитизация HTML в сервисе, автор — доверенный СУ). В админке: пагинация, даты создания/публикации, предпросмотр «как увидит бизнес». Бизнес видит опубликованное в кабинете (`Cabinet\AnnouncementController`, `/cabinet/news`, `/cabinet/updates`) — пагинированный список + **детальные страницы** (`…/news/{id}`, `…/updates/{id}`), новые сверху. Анонсы глобальные (без tenant_id). **Прочтение — пер-тенант** (`announcement_reads`, RLS): открытие раздела/детали помечает прочитанным; непрочитанное подсвечивает пункт меню (shared-prop `announcementsUnread`). Единая модель, тип (`news`/`update`) разводит две ленты. |
...
| GET | `/cabinet/news` · `/cabinet/updates` | `Cabinet\AnnouncementController` | Лента новостей/обновлений площадки (auth + `tenant`); открытие помечает анонсы прочитанными. Блокируется `EnsureCardAvailable`, если СУ поставил плашке «тех. работы». |
| GET/POST/PUT/DELETE | `/admin/news` · `/admin/updates` · `/admin/announcements[/{id}]` | `Admin\AnnouncementController` | Публикация анонсов супер-админом (auth + `super-admin`). |
