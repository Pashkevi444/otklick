...
| `App\Models\Flow` (`flows`, RLS) + `App\Services\FlowEngine` + `App\Http\Controllers\Cabinet\FlowController` | **Конструктор сценариев (no-code воронки)**: бизнес сам задаёт логику «если клиент написал X → ответь Y, предложи кнопки Z, сделай действие» без разработчиков. `flows.triggers` (jsonb) — фразы запуска; `flows.definition` (jsonb) — граф узлов `{start, nodes:{id:{text, action, options:[{label,next}]}}}`. `FlowEngine` встроен в `BotResponder` **после** автомата записи и **до** LLM: совпал триггер → ведёт диалог по кнопкам (`flow_state` в `conversations`), действие `start_booking`→`BookingFlow`, `escalate`→администратор, иначе свободный текст выходит из сценария к ИИ. Редактор `Cabinet/Scenarios/Index.vue` (узлы/кнопки/действия/старт-узел). Гейт `plan:flows` (Макс/Индивидуальный или оверрайд СУ) + раздел `scenarios` для сотрудников. |
