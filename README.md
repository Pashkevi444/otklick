...
| `App\Services\BookingFlow` | Пошаговая запись клиента в CRM: услуга → мастер (или «любой», `staff_id=0`) → день → конкретное время → телефон → `createBooking`. Каталог и слоты берутся ТОЛЬКО из CRM (источник истины); выбранный слот — строка `datetime` от CRM, возвращается без изменений (бот не выдумывает время). Состояние шага — `conversations.booking_state`. Фолбэки: нет подключения → `start()` возвращает `null` (обычное поведение/эскалация); нет услуг/слотов/сбой API → эскалация. Смену статуса (`booked`/`needs_human`) делает вызывающий слой по флагам `BotReply`. |
