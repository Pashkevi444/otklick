...
| `App\Models\Client` (`clients`, RLS) + `App\Models\ClientIdentity` (`client_identities`, RLS) + `ClientService` | **База клиентов**: единая карточка (имя, телефон, email, ник Telegram, первый канал, заметки, краткое LLM-резюме). Телефон — **кросс-канальный** ключ дедупа в пределах тенанта (`linkConversation` находит/создаёт клиента по телефону, проставляет `conversations.client_id`, дозаполняет пустые поля). **Узнавание вернувшегося — по нативной идентичности канала** (`client_identities`: `(tenant_id, channel_type, identity)` → client; identity = `external_chat_id`: Telegram chat_id, WhatsApp phone@c.us, VK/MAX user id) — `ClientService::recognizeReturning` (вызывается из `ContactCapture`) находит карточку по идентичности канала и заполняет контакты лида ИЗ КАРТОЧКИ (источник правды), без переспроса. Идентичность записывается при `linkConversation`. **Нормализация:** FK `conversations.client_id → clients.id` ON DELETE SET NULL (pgsql); `client_identities.client_id` ON DELETE CASCADE. `ClientService::delete` (транзакция): отвязывает лиды (`clearClientLinks`, история лида остаётся) и удаляет клиента → идентичности уходят каскадом, бот **«забывает»** человека (при следующем обращении — снова форма, а не «С возвращением»). **Источник правды для отображения лида:** грид/карточка «Лиды» показывают имя/телефон ИЗ карточки (по `client_id`), фолбэк на `conversations.contact_*` пока клиента нет. Канал — свойство лида (`channel_id`), не клиента. |
...
| `App\Services\ContactGate` | Контактная форма в начале диалога (едина для всех каналов): новому клиенту — приветствие + запрос **имени и телефона (обязательно), email (по желанию)** со СТРОГОЙ валидацией телефона (`PhoneExtractor::analyze` — короткий/длинный/мусор → просим исправить, а не «хаваем»). Собрали — даём кликабельные кнопки действий (`ReplyKeyboard`: «Записаться»/«Цены и услуги»/«Адрес и часы»). Вернувшегося узнаём по карточке клиента (нативная идентичность канала, `ClientService::recognizeReturning` заполняет контакты лида из карточки) — форму НЕ показываем, здороваемся по имени; удалили клиента из базы → бот забыл, снова форма. Имя из первого сообщения-вопроса НЕ вытаскиваем (только из ответа на форму). Флаг прохождения — `conversations.contacts_gate_done`; email — `conversations.contact_email` (→ `ClientService` в карточку). |
