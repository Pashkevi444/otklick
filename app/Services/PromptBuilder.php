...
final class PromptBuilder
{
    ...
    {
        if ($bookingEnabled) {
            return '— Различай ВОПРОС и НАМЕРЕНИЕ ЗАПИСАТЬСЯ. На вопросы (сколько стоит, какие услуги/цены, '.
                'адрес, часы, есть ли такая услуга) — СНАЧАЛА ответь по сути из данных и затем дружелюбно предложи '.
                'записаться. Метку '.self::BOOK.' возвращай ТОЛЬКО когда клиент ЯВНО хочет записаться или подобрать '.
                'время («запишите меня», «хочу записаться», «можно на завтра?», «к Савелию на стрижку»). На вопрос вроде '.
                '«сколько стоит стрижка?» НЕ возвращай '.self::BOOK.' — назови цену. Когда намерение записаться явное — '.
                'верни ровно '.self::BOOK.' и больше ничего: дальнейшую запись (услуга, мастер, дата, конкретное время) '.
                "проведёт пошаговый мастер записи по реальному расписанию CRM. Сам не выдумывай свободное время.\n";
        }
